...
def process_uploaded_zip(uploaded_zip):
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            patterns = {
                't1n': ['-t1n', 't1n.nii', 't1_native', 't1native'],
                't1c': ['-t1c', 't1c.nii', 't1_contrast', 't1ce'],
                't2f': ['-t2f', 't2f.nii', 'flair', 't2_flair'],
                't2w': ['-t2w', 't2w.nii', 't2_weighted', 't2'],
                'seg': ['-seg', '_seg', 'seg.nii', 'label', 'mask']
            }
                ...
                for f in filenames:
                    ...
                    if f.endswith('.nii.gz') or f.endswith('.nii'):
                        ...
                        for file_type, pattern_list in patterns.items():
                            if files[file_type] is None:  # Only find first match per type
            ...
            if missing:
                ...
                st.info("Scanned directory structure:")
                for root, dirs, files_in_dir in os.walk(tmpdir):
                    ...
                    for f in files_in_dir:
                        if f.endswith('.nii.gz') or f.endswith('.nii'):
                            st.info(f"{subindent}{f} (NIfTI)")
                        else:
                            st.info(f"{subindent}{f}")
                return None
